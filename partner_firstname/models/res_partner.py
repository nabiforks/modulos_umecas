...
class ResPartner(models.Model):
    ...
    @api.model
    def _get_computed_name(self, lastname, firstname,x_apellido_mat):
        ...
        if order == 'last_first_comma':
            ...
            return u" ".join((p for p in (firstname, lastname,x_apellido_mat) if p))
        else:
            return u" ".join((p for p in (lastname, firstname) if p))
    ...
    def _compute_name(self):
        ...
        for record in self:
            record.name = record._get_computed_name(
                record.lastname, record.firstname,record.x_apellido_mat
            )
